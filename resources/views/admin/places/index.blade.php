...
@section('create-btn')
    <a href="{{route("admin.places.create")}}" class="btn btn-primary btn-icon-split">
        ...
        <tbody>
            @foreach ($data as $index => $cgr)
                <tr>
                    ...
                    <td><a href="{{route('admin.places.show', $cgr)}}"><button class="btn btn-info">
                        <span class="icon text-white-50">
                            <i class="fas fa-eye"></i>
                        </span>    
                    </button></a></td>
                    <td><a href="{{route('admin.places.edit', $cgr)}}"><button class="btn btn-warning">
                        <span class="icon text-white-50">
                            <i class="fas fa-exclamation-triangle"></i>
                        </span>
                    </button></a></td>
                    <td>
                        <form action="{{route('admin.places.destroy', $cgr)}}" method="POST">
                            @csrf
                            @method('DELETE')
